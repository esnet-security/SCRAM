...
# Exported so compose.yml's own ${GOBGP_VERSION} substitution sees it too
export GOBGP_VERSION ?= v4.8.0
...
build: compose.override.yml
	@docker compose build --build-arg PYTHON_IMAGE_VER=$(PYTHON_IMAGE_VER) --build-arg POSTGRES_IMAGE_VER=$(POSTGRES_IMAGE_VER) --build-arg GOBGP_VERSION=$(GOBGP_VERSION) $(CONTAINER)
...
.Phony: copy-libs
copy-libs:
	@rm -rf translator/api
	@docker compose cp translator:/app/api translator/api
