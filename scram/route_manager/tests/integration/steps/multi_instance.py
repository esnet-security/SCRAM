...
def check_entry_inactive_on_secondary(context, ip):
    """Checks if entry is inactive by verifying it's NOT in the active entries.

    Note that this isn't whenwe run process_updates on this instance, so don't use this step out of context.
    If we mock websockets inthe future, then we can test this a bit better.
    """
    if not hasattr(context, "auth_token"):
    ...
    try:
        list_response = requests.get(
            f"{DJANGO_SECONDARY_URL}/api/v1/entries/",
            headers={"Authorization": f"Token {context.auth_token}"},
            timeout=10,
        )
        list_response.raise_for_status()
